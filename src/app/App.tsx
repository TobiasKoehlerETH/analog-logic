...
import { ChevronDown, GripVertical, X, RefreshCw } from "lucide-react";
...
const names: Record<string, string> = { uart: "UART / Serial", can: "CAN / CAN FD", i2c: "I²C", spi: "SPI", guess_bitrate: "Estimate bit rate", usb_signalling: "USB" };
...
export default function App() {
  ...
  const [mode, setMode] = useState<"pan" | "window">("pan");
  ...
  async function toggleStream() {
    ...
    try {
      if (running) {
        await invoke("stop_stream"); setRunning(false); setRefresh(r => r + 1);
      } else {
        ...
        firstFrame.current = true; setCapture(null); setResults([null, null]);
  ...
  useEffect(() => {
    ...
    const poll = async () => {
      try {
        ...
        if (!active) return;
        setCapture(view);
        if (view.error) setMessage(view.error);
        if (firstFrame.current && view.sample_count > 0) {
          setTime({ start: 0, span: Number(durationMs) / 1000 }); setVoltage(view.stats.map(fitStats)); firstFrame.current = false;
        }
        if (running && !view.running) setRunning(false);
  ...
  const protocolList = [...decoders].sort((a, b) => {
    const common = ["uart", "can", "i2c", "spi", "guess_bitrate", "usb_signalling"];
    ...
    <SidebarProvider className="h-svh min-h-0" style={{ "--sidebar-width": "17rem", "--header-height": "3.5rem" } as React.CSSProperties}>
          ...
          {order.map(channel => <SidebarGroup key={channel} className="px-4 pb-4" onDragOver={e => { if (dragChannel.current !== null) e.preventDefault(); }} onDrop={e => { e.preventDefault(); if (dragChannel.current !== null && dragChannel.current !== channel) setOrder(current => [...current].reverse()); dragChannel.current = null; }}>
            ...
            <SidebarGroupContent className="space-y-3">
              <div className="space-y-1.5"><Label htmlFor={`protocol-${channel}`} className="text-xs text-muted-foreground">Protocol</Label>
                <ScopeSelect id={`protocol-${channel}`} label={`A${channel + 1} protocol`} value={configs[channel].protocol} onValueChange={value => updateConfig(channel, { protocol: value, expression: "" })} options={[{ value: "off", label: "Off" }, ...(!decoders.length ? [{ value: "uart", label: "UART / Serial" }] : protocolList.map(d => ({ value: d.id, label: names[d.id] ?? d.name })))]} />
              </div>
      ...
      <SidebarInset className="min-w-0 overflow-hidden border md:my-2 md:mr-2">
        <header className="flex h-14 shrink-0 items-center gap-3 border-b px-4">
          ...
          <div className="ml-auto flex items-center gap-3"><span className="hidden text-xs text-muted-foreground sm:inline">{sampleRate} MS/s · {durationMs} ms window</span>
            <DropdownMenu><DropdownMenuTrigger asChild><Button variant="ghost" size="sm" className="text-xs" aria-label="Stream settings" disabled={running || busy}>Stream<ChevronDown className="size-3" /></Button></DropdownMenuTrigger><DropdownMenuContent align="end" className="w-56">
              ...
              <DropdownMenuSub><DropdownMenuSubTrigger>Display window · {durationMs} ms</DropdownMenuSubTrigger><DropdownMenuSubContent><DropdownMenuRadioGroup value={durationMs} onValueChange={setDurationMs}>{WINDOW_LENGTHS.map(option => <DropdownMenuRadioItem key={option.value} value={option.value}>{option.label}</DropdownMenuRadioItem>)}</DropdownMenuRadioGroup></DropdownMenuSubContent></DropdownMenuSub>
              <DropdownMenuSeparator /><DropdownMenuItem onSelect={() => setSettingsOpen(true)}>Custom stream settings…</DropdownMenuItem>
            </DropdownMenuContent></DropdownMenu>
            <Button size="sm" disabled={busy || (!running && !devices.length)} onClick={toggleStream} aria-label={running ? "Stop stream" : "Start stream"}>{busy ? (running ? "Stopping…" : "Starting…") : running ? "Stop" : "Start"}</Button>
          </div>
        </header>
